...
static ESP32RMTCustomController * gControllers[FASTLED_RMT_MAX_CONTROLLERS];

// -- Current set of active controllers, indexed by the RMT
//    channel assigned to them.
static ESP32RMTCustomController * gOnChannel[FASTLED_RMT_MAX_CHANNELS];
...
int ESP32RMTCustomController::gMaxChannel;
int ESP32RMTCustomController::gMemBlocks;


ESP32RMTCustomController::ESP32RMTCustomController(int DATA_PIN, int T1, int T2, int T3, int maxChannel, int memBlocks)
    : mPixelData(0),
      mSize(0),
      mCur(0),
      mWhichHalf(0),
...
uint8_t * ESP32RMTCustomController::getPixelBuffer(int size_in_bytes)
{
    if (mPixelData == 0) {
        mSize = size_in_bytes;
...
void ESP32RMTCustomController::init(gpio_num_t pin)
{
...
void IRAM_ATTR ESP32RMTCustomController::showPixels()
{
    if (gNumStarted == 0) {
        // -- First controller: make sure everything is set up
        ESP32RMTCustomController::init(mPin);
    ...
    if (gNumStarted == gNumControllers) {
        ...
        while (channel < gMaxChannel && gNext < gNumControllers) {
            ESP32RMTCustomController::startNext(channel);
...
void IRAM_ATTR ESP32RMTCustomController::startNext(int channel)
{
    if (gNext < gNumControllers) {
        ESP32RMTCustomController * pController = gControllers[gNext];
...
//    for it to finish.
void IRAM_ATTR ESP32RMTCustomController::startOnChannel(int channel)
{
...
void IRAM_ATTR ESP32RMTCustomController::tx_start()
{
...
void IRAM_ATTR ESP32RMTCustomController::doneOnChannel(rmt_channel_t channel, void * arg)
{
    ESP32RMTCustomController * pController = gOnChannel[channel];
...
void IRAM_ATTR ESP32RMTCustomController::interruptHandler(void *arg)
{
...
void IRAM_ATTR ESP32RMTCustomController::fillNext(bool check_time)
{
...
void ESP32RMTCustomController::initPulseBuffer(int size_in_bytes)
{
    if (mBuffer == 0) {
...
//    This function is only used when the built-in RMT driver is chosen
void ESP32RMTCustomController::convertByte(uint32_t byteval)
{
